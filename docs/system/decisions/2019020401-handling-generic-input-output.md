...
The process' input and output are usually used by adapters of the application running on that process: in this example, for instance, the input would be handled by the sensor adapter, when signals are simulated from the command line, while the output would be used by the message bus adapter for example.

We might think that the operating system the process is running on, is a device in its own rights, but that's not actually correct, because the term "device" doesn't relate to any specific system, but to a class of systems sharing a protocol: we could also use the term "actor" for it. Thus, the operating system is not a device, rather a technical system over which certain devices, or actors, might run.
