...
test('holds up to 16 bits', () => {
    expect(Word.MAX).toBe(65535);
});
...
test('if constructing with one argument, it must be integer', () => {
    expect(() => new Word(new Byte(10))).toThrow('Word must be constructed with one integer or two bytes');
});
